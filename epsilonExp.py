...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            ...
            sess.close()
            return sparsity, rmse, mae, mape

if(__name__ == "__main__"):
    rmseAll, maeAll, mapeAll = [], [], []
    results = []
    for epsilon in [2, 1, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001]:
        tf.reset_default_graph()
        print("*"*50, "Starting Experiment with Epsilon = ", epsilon, "*"*50)
        params.threshold = epsilon
        sparsity, rmse, mae, mape = train()
        results.append([epsilon, sparsity, rmse, mae, mape])
    resultsDF = pd.DataFrame(results, columns=["Epsilon", "Sparsity", "RMSE", "MAE", "MAPE"])
    resultsDF.to_csv("epsilonExpResults.csv", index=False)
